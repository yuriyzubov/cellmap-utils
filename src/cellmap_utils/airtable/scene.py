...
def upsert_record_scene_tables(scene_table : api.table.Table,
    ...
    if len(image_records) > 1:
        raise ValueError("Multiple images with the same path are found in airtable image table. Only one record should exist!")
    else:
    ...
    scene_to_image_insert = {'image': [image_airt['id']],
                            'contrast_start': 0,
                                'contrast_stop': 255,
                                    'color': 'white'}
    ...
    existing_record_scene_to_image = scene_to_image_table.first(formula = match({'location_web' : location }))

    # add records to scene_to_image table
    if existing_record_scene_to_image == None:
        scene_to_image_ids[location] = scene_to_image_table.create(scene_to_image_insert)
    else:
        scene_to_image_ids[location] = scene_to_image_table.update(existing_record_scene_to_image['id'], scene_to_image_insert)
    ...
    if existing_record_scene_to_image:
        existing_record_scene = scene_table.first(formula = match({'scene_to_image' : existing_record_scene_to_image['fields']['name']}))
    else:
        existing_record_scene = None
        
    scene_insert = {'name': 'Default view' ,'scene_to_image': [scene_to_image_ids[location]['id']],
    ...
    if existing_record_scene == None:
        ...
        scene_ids[location] = scene_table.update(existing_record_scene['id'], scene_insert)
