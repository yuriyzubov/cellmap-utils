...
from pydantic_models import (SupaDatasetModel,
...
def get_image_record(image_path: str,
                                        ds_name : str,
                                        at_api: api):
    ...
    try:
        ...
        if image_record['fields']['image_type'] == 'em':
            ...
            source = {
                ...
                "duration_days": (datetime.strptime(fibsem_record['fields']['stop_date'], format_string).date()
                                                - datetime.strptime(fibsem_record['fields']['start_date'], format_string).date()).days,
    ...
    supa_image = SupaImageModel(
        ...
        grid_dims= ['z', 'y', 'x'],
        grid_scale= [image_record['fields'][f'resolution_{_}_nm'] for _ in ['z', 'y', 'x']],
        grid_translation= [image_record['fields'][f'offset_{_}_nm'] for _ in ['z', 'y', 'x']],
        grid_units= ['nm', 'nm', 'nm'],
        grid_index_order= 'C',
        stage= 'dev',
...
def get_img_acq_record(ds_name : str,
                                        at_api: api):
    
    fibsem_table = at_api.table(os.environ['AIRTABLE_BASE_ID'], os.environ['FIBSEM_TABLE_ID'])
    fibsem_record = fibsem_table.all(formula=match({'name' : ds_name}))

    sample_table = at_api.table(os.environ['AIRTABLE_BASE_ID'], os.environ['SAMPLE_TABLE_ID'])
    ...
    supa_acquisition = SupaImageAcquisitionModel(
        ...
        grid_axes = ['z', 'y', 'x'],
        grid_spacing = [fibsem_record['fields'][f'resolution_{_}_nm'] for _ in ['z', 'y', 'x']],
        grid_spacing_unit = 'nm',
        grid_dimensions = [image_record['fields'][f'size_{_}_pix'] for _ in ['z', 'y', 'x']],
        grid_dimensions_unit = 'nm',
    )
    
    return supa_acquisition

def get_sample_record(ds_name : str,
                                        at_api: api):
...
def get_dataset_record(ds_name : str,
                        at_api: api):
...
def get_dataset_full(img_path : str,
                     ds_name : str,
                     at_api : api):

    return {'image' : get_image_record(img_path, ds_name, at_api),
